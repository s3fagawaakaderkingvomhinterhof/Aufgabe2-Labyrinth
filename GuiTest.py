...
class GridFrame(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        labyrinth_obj = Labyrinth(self.columns, self.rows)
        ...
        self.SetSize(
            (columns_with_borders * self.edge_length + 16, rows_with_borders * self.edge_length + 39))  # wow cool shit
        ...
        if self.click_counter % 2 == 1:
            ...
            if self.dest_pos[0] == -1 and self.dest_pos[1] == -1:
                ...
                if self.dest_pos[1] > 0 and self.dest_pos[0] > 0 \
                    ...
                    self.grid.SetCellBackgroundColour(self.dest_pos[1], self.dest_pos[0],
                                                      wx.BLUE)  # set end for 1st click
                    self.grid.SetCellValue(self.dest_pos[1], self.dest_pos[0], self.string_end)
                    self.agent.set_end(self.dest_pos[0], self.dest_pos[1])
                    self.grid.ForceRefresh()
                else:
                    print('go 3. else')
                    self.clear_labyrinth()
                ...
                if self.dest_pos[1] > 0 and self.dest_pos[0] > 0 \
                        and self.dest_pos[1] < self.rows + 1 and self.dest_pos[0] < self.columns + 2:
                    self.grid.SetCellBackgroundColour(self.dest_pos[1], self.dest_pos[0],
                                                      wx.BLUE)  # set end from 2nd click and so on
                    self.grid.SetCellValue(self.dest_pos[1], self.dest_pos[0], self.string_end)
                    self.agent.set_end(self.dest_pos[0], self.dest_pos[1])
                    self.grid.ForceRefresh()
                else:
    ...
    def clear_labyrinth(self):
        ...
        self.grid.SetCellValue(0, 0, 'reset')
        self.grid.SetCellTextColour(0, 0, wx.RED)
        self.labyrinth_model.clear_labyrinth()
